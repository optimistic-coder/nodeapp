...
router.post('/add',(req,res)=>{
 ...
 Users.findOne({username:req.body.username},function(err,result){
     ...
     if(result){
        res.json({result:0,message:"User name alredy exist"});
     }else{
        var imagepath= [];
        var filearray = [];
        filearray.push(req.body.image);        
        imagepath = saveImg(filearray);
        if(imagepath.length != 0){
            req.body.image= imagepath[0];          
        }  
        const newcat = new Users(req.body);
...
router.get('/index',async(req,res)=>{
    const list = await Users.find({status:1}).sort({_id:-1});
        res.json(list);
    })
